...
def insert_data(session, model, score, cv):
    ...
    cur.execute('INSERT INTO submit_data VALUES (?, ?, ?, ?, ?)',
                (f'{session}/{model}', session, model, score, cv))
    conn.commit()
    conn.close()


def run_submit(command, session, model, cv):
    now = time.time()
    ...
    insert_data(session, model, score, cv)
...
if __name__ == "__main__":
    ...
    for i in range(s_df.shape[0]):
        ...
        full_command = f"nsml submit {full_session} {int(m)} --esm {nsml_name}"
        li_procs.append(Process(target=run_submit, args=(full_command, s, m, cv)))
